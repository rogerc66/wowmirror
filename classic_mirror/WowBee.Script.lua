				...
				elseif level==2 then
					...
					if Level1_Key=="LISTITEM_RIGHTCLICK_ADVANCED_TITLE" then					
						...
						info.func=function(c)
							...
							WowBeeHelper_OnScript(string.format("PickupMacro(CreateMacro(\"Run_%s_%s\",GetMacroIndexByName(\"%s\"),\"/Bee Run %s %s\",1));",self.SchemeInfo.Name,self.SchemeInfo.Items[index].GUID,icon or 1,self.SchemeInfo.Name,self.SchemeInfo.Items[index].GUID))
						end;
						...
						info.func=function(c)
							...
							WowBeeHelper_OnScript(string.format("PickupMacro(CreateMacro(\"Auto_%s_%s\",GetMacroIndexByName(\"%s\"),\"/Bee [mod:alt]Stop;[btn:2]Stop;Auto %s %s\",1));",self.SchemeInfo.Name,self.SchemeInfo.Items[index].GUID,icon or 1,self.SchemeInfo.Name,self.SchemeInfo.Items[index].GUID))
						end;
						UIDropDownMenu_AddButton(info,level);
						...
						info.func=function(c)
							...
							WowBeeHelper_OnScript(string.format("PickupMacro(CreateMacro(\"STOP\",GetMacroIndexByName(\"Ability_Hunter_BeastCall\"),\"/Bee Stop\",1));"))
						end;
						UIDropDownMenu_AddButton(info,level);
						return;
					end
...
function WowBeeHelper_OnMacro(script,name)
	...
	--print(WowBee.Event.Encode(2,tonumber(t)));
	--print(t);
	return k;
end
	...
	if (GetTime() - WowBee.Event.SetSleep >3) then
		...
		elseif WowBee.Event.Action==1 then
			if WowBee.Event.Verify > 0 then
				...
				if(WowBee.Event.VerifyRetry<120) then
