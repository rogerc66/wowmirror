...
function BeeRangeF()
   local rc = LibStub("LibRangeCheck-2.0")
   local minRange, maxRange = rc:GetRange('focus')
   if not minRange then
       print("cannot get range estimate for focustarget")
   elseif not maxRange then
       print("focustarget is over " .. minRange .. " yards")
       return minRange;
   else
       print("focustarget is between " .. minRange .. " and " .. maxRange .. " yards")
       return minRange;
   end
end

function BeeRangeR(unit1, unit2)-- 判断距离
	if not unit2 then
		...
		local jl;
		if rc then
		--print("rc check is true")
		_,jl = rc:getRange(unit1)			
		end
		--print("unit1 is " .. unit1)
...
function BeeUnitCastSpellTime(unit) --获得指定目标正在施放的法术剩馀时间
	...
	local spell, _, _, startTime, endTime = CastingInfo(unit)
	...
	local spellch, _, _, startTime, endTimech = ChannelInfo(unit)
	if spellch then 
...
function LeaveCombatCheck()
	if not leaveCheck then
		leaveCheck = CreateFrame("Frame")
	end
		
	local frame = leaveCheck
...
local y,n,s,_ for i=1,40 do n,_,_,_,s=UnitAura("target",i)
